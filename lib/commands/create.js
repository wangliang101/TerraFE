  ...
  const questions = [
    ...
    },
    {
      ...
      choices: () => {
        ...
        const detected = npmUtils.detectPackageManager(targetDir);
        const choices = [
          { name: `🤖 自动选择 (当前: ${detected})`, value: 'auto' },
          { name: '📦 npm', value: 'npm' },
      ...
      default: options.packageManager || config.get('packageManager', 'auto'),
    },
    {
      type: 'confirm',
      name: 'gitInit',
...
async function executeGeneration(projectName, options) {
  ...
  // 智能选择包管理器
  const packageManager = npmUtils.selectPackageManager(targetDir, options.packageManager || 'auto');
  ...
  logger.info(`   包管理器: ${chalk.yellow(packageManager)}`);
